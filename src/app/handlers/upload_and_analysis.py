...
def register_callbacks_02(app, controller, embedder=None):
    ...
    # =========================================================
    # Dropdowns dependientes de doc-context
    # =========================================================
    ...
    def update_analysis_doc_selector(doc_ctx):
        if not isinstance(doc_ctx, dict):
            return [], None
        options = [
            {"label": ctx.get("file_name", doc_id), "value": doc_id}
    ...
    def update_analysis_target(doc_ctx):
        if not isinstance(doc_ctx, dict):
            return [], None
        options = [
            {"label": ctx.get("file_name", doc_id), "value": doc_id}
    ...
    def handle_upload_and_analysis(contents, n_clicks, filenames, upload_ctx,
                                   selected_doc_id, fast_mode):
        ...
        if triggered == "upload-pdf":
            ...
            options = [{"label": i["file_name"], "value": i["doc_id"]} for i in items]
            selected = [i["doc_id"] for i in items]

            return items, dash.no_update, f"Uploaded {len(items)} file(s)", dash.no_update, options, selected, True
        ...
        if triggered == "run-analysis":
            ...
            items = upload_ctx if isinstance(upload_ctx, list) else []
            ...
            def process_pdf(target):
                ...
                for p in pages:
                    for b in p.get("blocks", []):
                        ...
                        classified_blocks.append({**b, "page": p.get("page_number")})
                        if field_info:
            ...
            with ThreadPoolExecutor(max_workers=4) as ex:
                futures = [ex.submit(process_pdf, item_map[i]) for i in selected_ids if i in item_map]
            ...
            if first and first.get("overlays"):
                try:
                    with open(first["overlays"][0]["path"], "rb") as f:
                        img_src = "data:image/png;base64," + base64.b64encode(f.read()).decode()
                except Exception:
                    pass

            options = [{"label": ctx["file_name"], "value": did} for did, ctx in results.items()]
            summary = f"Processed {len(results)} document(s)"

            selected_value = options[0]["value"] if options else None

            return dash.no_update, results, summary, img_src, options, selected_value, False
