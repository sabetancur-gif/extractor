    ...
    return dbc.Card(
        dbc.CardBody(
            [
                ...
                html.Div(f"Páginas: {ctx.get('pages_total', len(ctx.gen('pages', [])))}", className="mb-1"),
                html.Div(f"Campos: {len(ctx.get('fields', []))}", className="mb-1"),
                html.Div(f"Bloques: {len(ctx.get('classified_blocks', []))}", className="mb-1"),
                html.Div(f"Modo: {ctx.get('processing_mode', 'N/A')}", className="text-muted small"),
            ]
        ),
